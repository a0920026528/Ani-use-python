...
        if chat_history:
            summary = " ".join([msg.split("：")[1] if "：" in msg else msg for msg in chat_history if len(msg) < 100])[:200] # Improved summary, only content
        else:
            summary = "平靜的一天，Ani 想念哥哥～" # Avoid empty summary
        new_event = {
...
        memory_json = json.dumps({"longterm_memory": self.timeline}, ensure_ascii=False, indent=2)
        print(f"生成的記憶 JSON：\n{memory_json}\n") # Emphasize print
        return memory_json
